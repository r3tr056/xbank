...
class HmacSHA256JWTService implements JWTSerializer, JWTDeserializer {
	...
	private static final Pattern JWT_PATTERN = compile(format("^(%s\\.)(%s\\.)(%s)$", BASE64URL_PATTERN, BASE64URL_PATTERN, BASE64URL_PATTERN));
	...
	public JWTPayload jwtPayloadFromJWT(String jwtToken) {
		...
		final var splindedTokens = jwtToken.split("\\.");
		if (!splindedTokens[0].equals(JWT_HEADER)) {
			throw new IllegalArgumentException("Malformed JWT! Token must starts with header : " + JWT_HEADER);
		}

		final var signatureBytes = HmacSHA256.sign(secret, splindedTokens[0].concat(".").concat(splindedTokens[1]));
		if (!base64URLFromBytes(signatureBytes).equals(splindedTokens[2])) {
			throw new IllegalArgumentException("Token has invalid signature : " + jwtToken);
		}

		try {
			final var decodedPayload = stringFromBase64URL(splindedTokens[1]);
