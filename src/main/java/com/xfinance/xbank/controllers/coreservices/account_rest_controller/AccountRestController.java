package com.xfinance.xbank.controllers.coreservices.account_rest_controller;

import com.xfinance.xbank.constants.constants;
import com.xfinance.xbank.models.Account;
import com.xfinance.xbank.services.AccountService;
import com.xfinance.xbank.utils.AccountInput;
import com.xfinance.xbank.utils.CreateAccountInput;
import com.xfinance.xbank.utils.InputValidator;
...
public class AccountRestController {
    ...
    public ResponseEntity<?> checkAccountBalance(@Valid @RequestBody AccountInput accountInput) {
        ...
        if (InputValidator.isSearchCriteriaValid(accountInput)) {
            // Attempt to retrive the account info
    ...
    public ResponseEntity<?> createAccount(@Valid @RequestBody CreateAccountInput createAccountInput) {
        ...
        if (InputValidator.isCreateAccountCriteriaValid(createAccountInput)) {
            Account account = accountService.createAccount(createAccountInput.getNameName(), createAccountInput.getOwnerName());
            // Return the account details, or warn that no account was found for given input
            if (account == null) {
